...
function printMark(number) {
    ...
    if (box.innerText == "") //checking for the empty ones
    {
        ...
        winnerOfGame();
        checkSign();
        disp.innerHTML = "<center>" + sign + "  's Turn" + "</center>"; //displays which players's turn
    }
}
...
//A handy function that will be used furthur
function getBox(no) {
...
function winnerOfGame() {
    if (
        ...
        disp.innerHTML = "<center>" + sign + " Won This Match" + "</center>";
        for (let i = 1; i <= 9; i++) {
